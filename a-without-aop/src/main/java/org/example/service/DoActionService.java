...
public class DoActionService {
    ...
    public void doAction(){
        ...
        PerformanceLoggerInfo performanceLoggerInfo=logger
                .start("reportProvider.provideReport()");
        Report report=reportProvider.provideReport();
        logger.end(performanceLoggerInfo);

        performanceLoggerInfo=logger
                .start("reportRepository.saveReport(report)");
        reportRepository.saveReport(report);
        logger.end(performanceLoggerInfo);

        performanceLoggerInfo=logger
                .start("");
        reportSummary.summaryReport(LocalDate.of(2023,4,10),
                LocalDate.now());
        logger.end(performanceLoggerInfo);
